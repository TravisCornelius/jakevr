...
public class ZoomCamera : MonoBehaviour {
	...
	void Start()
	{
		CameraScript = GameObject.Find ("Player").GetComponent<CameraManager>();
		canZoom = true;
	}
	...
	void Update () {
		...
		if (Input.GetKeyDown (KeyCode.Mouse1) && !zooming && !CameraScript.broken && canZoom) {
			if (!zoomed) {
				StartCoroutine (zoomming ());
				zoomed = true;
				this.GetComponent<Animation> ().Play ("ZoomIn"); 
				this.GetComponent<AudioSource> ().PlayOneShot (ZoomIn, volume);
			} else {
				StartCoroutine (zoomming ());
				zoomed = false;
		...
		else if (Input.GetKeyDown (KeyCode.Mouse1) && !zooming && CameraScript.broken) 
		{
